...
contract FlightSuretyApp {
    ...
    uint256 public constant INSURANCE_CAP = 1 ether;
    uint256 public constant ACTIVATION_FEE = 10 ether;
    ...
    // Define a modifier that checks the price and refunds the remaining balance
    modifier checkValueLimit(uint _price) {
        _;
        uint price = _price;
        if(msg.value>_price) {
            uint amountToReturn = msg.value - price;
    ...
    */   
    ...
    function activateAirline(
        ...
        payable
        requireIsOperational
        paidEnough(ACTIVATION_FEE)
        checkValue(ACTIVATION_FEE)
        returns(bool success)
    {
        return flightSuretyData.activateAirline(msg.sender, mode);
    }
    ...
    {
        return flightSuretyData.fetchFlightBuffer(_airline, _flight);
    }
